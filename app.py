...
def envoyer_notifications(titre, message):
    import time as _t; print(f"[DEBUG PUSH] appel envoyer_notifications a {_t.time()}")
    if not PUSH_ACTIF:
        return
...
def enregistrer_signalement(type_signalement):
    ...
    signalement_id = cursor.lastrowid
    ...
    envoyer_notifications(
        ...
        json.dumps({
            "title": "Halé Bou Rér",
            "body": "Nouveau signalement disponible",
            "url": f"/detail/{signalement_id}"
        })
    )
